...
from ebor.Envs.base_env import BallEnv, BoxEnv
...
class BallGym(BallEnv):
    ...
    def reset(self, is_random=True, remove_collision=True):
        ...
        # initialise objects
        if is_random:
            balls_dict = sample_example_positions(self.num_per_class, self.catetory_list, pattern='Random2D', bound=self.bound, r=self.r)
...
class BoxGym(BoxEnv):
    def __init__(self, pattern='Stacking', auto_flatten=False, **kwargs):
        ...
        self.action_space = Dict({
            "pick": Box(-1, 1, shape=(2,), dtype=np.float32),
            "place": Box(-1, 1, shape=(3,), dtype=np.float32),
            "id": Discrete(self.num_objs),
        })
    ...
    def flatten_states(self, states_list):
        """
        input: [state1, state2, ...]
        output: nparr, shape [num_states, (3+1)*num_objs]
        """
        states_np = []
        for state in states_list:
            state_np = []
    ...
    def flatten_actions(self, actions_list):
        """
        input: [action1, action2, ...]
        output: nparr, shape [num_actions, 2+3+1]
        """
        actions_np = []
        for action in actions_list:
            action_np = []
            action_np.append(action['pick'])
            action_np.append(action['place'])
            action_np.append(np.array(action['id']).astype(np.int64))
            action_np = np.concatenate(action_np, axis=0)
            actions_np.append(action_np)
        return np.stack(actions_np, axis=0)
    ...
    def unflatten_states(self, states_np):
        ...
        for state_np in states_np:
            state_dicts = OrderedDict()
            for idx in range(self.num_objs):
                obj_dict = {"position": state_np[idx*4:idx*4+3], "category": state_np[idx*4+3].astype(np.int64).item()}
                state_dicts[f'obj{idx}'] = obj_dict
            states_list.append(state_dicts)
        return states_list
    ...
    def unflatten_actions(self, actions_np):
        ...
        for action_np in actions_np:
            ...
            action_dict['pick'] = action_np[:2]
            action_dict['place'] = action_np[2:2+3]
            action_dict['id'] = action_np[-1].astype(np.int64)
            actions_list.append(action_dict)
        return actions_list

    def nop_action(self):
        """
        action space : {'pick': Box(2), 'place': Box(3), 'id': Discrete(self.num_objs)}
        """
        return {'pick': np.zeros(2, ), 'place': np.zeros(3,), 'id': -1}
